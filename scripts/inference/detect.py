...
def run(model, frame):
    ...
    h, w = frame.shape[:2]
    # task='segment' is required here too — ensures ONNX uses the seg head
...
def main():
    ...
    if mp.suffix.lower() == ".onnx":
        ...
        print(f"  💡 If still 0 detections, test with best.pt first:")
        print(f"     python detect.py --model best.pt --source <same images>")
    print(f"{'='*65}")

    # ── CRITICAL: ONNX files don't store task metadata ───────────────────────
    # Without task='segment', ultralytics defaults to 'detect' and the
    # segmentation head is completely ignored → 0 detections always.
    if mp.suffix.lower() == ".onnx":
